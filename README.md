...
- [ ] multi-seed, multi-domain grid once a real corpus exists
...
- [ ] 20-trace manual inspection (AGENTS.md §8) — required before any claim

### Demo results (toy corpus, cutoff 2022-12-31, seed 42, budget 16)

Provisional automated rubric (`countergap_auto_v1`, not human review):
...
it proposed a specific claim, generated falsification queries, searched the
frozen corpus for pre-cutoff counter-evidence, judged the claim to survive,
and challenged the final hypothesis with both a support and a counter search
before stopping. The one-shot LLM produced the same style of claim but could
not validate it — exactly the contrast the environment is designed to expose.
Interpretation caveats: LLM runs are non-deterministic (single seeds shown);
the auto rubric rewards vague claims with high "novelty" (see
`score_trace.py` notes); future-emergence is automated term overlap only.
