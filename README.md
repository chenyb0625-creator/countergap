...
### M3 — Real corpus — DONE (v1 snapshot)

- [x] OpenAlex (CC0) snapshot: "language model agents", 2019-2025 → 326 docs
- [x] versioned provenance (`data/llm_agent_eval_v1.meta.json`), deterministic split
- [x] full 8-method suite ran on the real corpus (cutoff 2024-06-30, 142 pre / 184 post)
- [ ] multi-cutoff grid and 20+ real-corpus traces before any claim
...
### Demo results (real corpus — OpenAlex CC0, cutoff 2024-06-30, seed 42, budget 16)
...
|---|---|---|---|---|---|---|---|---|
| random | insufficient_evidence | 5 | 0.97 | 0.00 | 0.30 | 0.01 | 1.0 | 0.420 |
| keyword_trend | insufficient_evidence | 15 | 0.97 | 0.02 | 0.30 | 0.03 | 1.0 | 0.427 |
| one_shot | insufficient_evidence | 6 | 0.97 | 0.00 | 0.30 | 0.03 | 1.0 | 0.420 |
| no_counter_search | insufficient_evidence | 10 | 0.98 | 0.00 | 0.30 | 0.03 | 1.0 | 0.424 |
| counter_search | insufficient_evidence | 9 | 0.98 | 0.00 | 0.30 | 0.03 | 1.0 | 0.424 |
| embedding_boundary | insufficient_evidence | 13 | 0.92 | 0.06 | 0.30 | 0.11 | 1.0 | 0.436 |
| one_shot_llm (DeepSeek) | insufficient_evidence | 6 | 0.93 | 0.04 | 0.30 | 0.07 | 0.5 | 0.376 |
| llm_counter_search (DeepSeek) | **no_validated_gap (falsified)** | 13 | 0.95 | 0.02 | **1.00** | 0.06 | 0.5 | 0.550 |

On real literature the interactive LLM agent performed a textbook
**counterexample discovery**: it proposed a gap about evaluating LLM agents'
self-reflection/correction, ran falsification searches, found two pre-cutoff
papers already covering the topic, revised, then **rejected** the claim with a
recorded reason (`no_validated_gap`, robustness 1.0). The one-shot LLM
proposed the same style of claim without any falsification step. This is the
mechanism the environment was built to expose — but it is a single run, not
a statistical result; see `docs/trace_inspection.md` for the toy-corpus audit
and the pending real-corpus inspection.
